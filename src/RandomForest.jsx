...
export default function RandomForest() {
 ...
 useEffect(() => {
  ...
  const fetchData = async () => {
    try {
      const res = await myAxios.get("http://localhost:8085/runRF");
  ...
  const fetchImg = async () => {
    try {
      const res = await myAxios.get("http://localhost:8085/getGraph",{
  ...
  return(
    ...
    <div className='graph'>
    <div className='heading'>Random Forest (Actual vs Prediceted) Graph</div>
    {displayGraph()}
   </div></div>
    ):(
