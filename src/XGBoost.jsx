...
export default function XGBoost() {
 ...
 useEffect(() => {
  ...
  const fetchData = async () => {
    try {
      const res = await myAxios.get("http://localhost:8087/runXGBoost");
  ...
  const fetchImg = async () => {
    try {
      const res = await myAxios.get("http://localhost:8087/getGraph",{
  ...
  return(
    <div className='body'>
      ...
      (
        <div>
        <div className='heading'>ErrorMetrics ( XGBoost )</div>
    ...
    <div className='graph'>
    <div className='heading'>XGBoost (Actual vs Prediceted) Graph</div>
    {displayGraph()}
   </div></div>
    ):(
